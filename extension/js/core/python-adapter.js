...
function run(runtime, request, options, callback) {
  ...
  try {
    fs.writeFileSync(requestPartial, JSON.stringify({ engine: runtime.engine, modelPath: request.modelPath, audioPath: request.audioPath, language: request.language, device: request.device, deviceIndex: request.deviceIndex || 0, computeType: request.computeType, outputPath: outputPath, vad: request.vad !== false, vadModelPath: request.vadModelPath || null, allowHybrid: request.allowHybrid === true, modelSizeBytes: request.modelSizeBytes || null, speechRegions: Array.isArray(request.speechRegions) ? request.speechRegions : [], speechBoundaryHints: Array.isArray(request.speechBoundaryHints) ? request.speechBoundaryHints : [], decodeRegions: Array.isArray(request.decodeRegions) ? request.decodeRegions : [], speechRegionPaddingMs: request.speechRegionPaddingMs === undefined ? 240 : request.speechRegionPaddingMs, speechRegionMergeGapMs: request.speechRegionMergeGapMs === undefined ? 180 : request.speechRegionMergeGapMs, initialPrompt: request.initialPrompt || null, hotwords: request.hotwords || null, beamSize: request.beamSize || 5, temperatureFallback: request.temperatureFallback !== false }), "utf8");
