...
test("Source Text keys keep time, style and expressions while each text is replaced",()=>{const h=harness(1),p=h.template.layer(1).doc;p.keys=[{time:0.2,value:{text:"Old",fontSize:18}},{time:3.4,value:{text:"Old",fontSize:62}}];p.expression="value";p.expressionEnabled=true;h.template.layer(1).locked=true;assert.equal(h.create().data.count,5);const l=h.placed[0].source.layer(1);assert.deepEqual(l.doc.keys,[{time:0.2,value:{text:"Text 1",fontSize:18}},{time:3.4,value:{text:"Text 1",fontSize:62}}]);assert.equal(l.doc.expression,"value");assert.equal(l.locked,true);assert.equal(l.inPoint,0.5);assert.equal(l.outPoint,4);assert.equal(p.keys[0].value.text,"Old");});
